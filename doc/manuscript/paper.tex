...
data-driven Plethora of methods, but only handfull that can be used on different
organisms and have been validated on simulated data set. Ensemble methods are
more challenging to use, to validate and to explore, but offer more
biologically accurate view of what is going on in teh nucleus. Overall,
powerfull tool but that still needs to be extended oto work on challenging
data set. 

model-driven methods are organisms and study specific. For each data set, the
set of constraints need to be revisited. Yet, once built, they can be used ot
replace high throughput FISH experiments at low cost.

Downstream analysis gave important insights in the relation between gene
regulation and genome architecture of teh {\em P. falciparum}. Yet a lot
remains to be done. How to compare in a meaningful manner structures from
different time points at a genome-scale level? 
